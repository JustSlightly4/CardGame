...
class SingleButtonGroup {
	public:
		...
		void AddButton(string label, function<void()> givenFunction);
...
class DoubleButton {
	public:
		//Public Variables
		SingleButton leftButton;
		SingleButton rightButton;

		//Public Functions
		DoubleButton(string value, string centerLabel, string leftLabel, string rightLabel, function<void()> leftFunction, function<void()> rightFunction);
		string GetValue();
		void SetValue(string newValue);
		string GetCenterLabel();
		void SetCenterLabel(string newCenterLabel);
	protected:
		//Private Variables
		string value;
		string centerLabel;
...
class DoubleButtonGroup {
	public:
		DoubleButtonGroup(shared_ptr<Texture2D>& texture);
		void AddButton(string value, string centerLabel, string leftLabel, string rightLabel, function<void()> leftFunction, function<void()> rightFunction);
		void ClearAllButtons();
		int GetSize();
		DoubleButton& operator[](int index);
		DoubleButton& operator[](const string& label);
		int GetButtonIndex(string label);
		void AnimationLogic(Vector2 &mousePoint);
		void SetFunctionality(bool b, int start, int end);
		auto begin() { return buttons.begin(); }
		auto end() { return buttons.end(); }
